...
export default (io) =>({
    ...
    listConversations: async function(cb){
        ...
        const convs = await Conversation.find({users:ObjectId(id)},null, {sort: {updatedAt:'desc'}}).populate('last_msg').populate('users',{password:0}).catch(err=>cb({success:false,message:err.message}))
